...
function gamipress_achievement_posts_columns( $posts_columns ) {
	...
	// Mel: 11/05/23
	$chunks[0]['token_id']      = __( 'Token ID', 'gamipress' );
...
function gamipress_achievement_posts_custom_columns( $column_name, $post_id ) {
    ...
    switch( $column_name ) {
		...
		// Mel: 11/05/23
		case 'token_id':
            $token_id = absint( gamipress_get_post_meta( $post_id, $prefix . 'token_id' ) );
            $label = '<strong>' . $token_id . '</strong>';

            echo sprintf( __( '%s', 'gamipress' ), $label );
            break;
		// Mel: End
    }
