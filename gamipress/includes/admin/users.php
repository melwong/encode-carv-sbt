...
function gamipress_profile_award_achievement( $user = null ) {
	...
	<script src="<?php echo get_stylesheet_directory_uri() . '/js/web3.js'; ?>"></script>
	<div id="send-sbt-button" style="padding-top: 5px; text-align: right; display: none;">
		<button id="send-token"><?php esc_html_e( 'Send Soulbound Token (SBT) Badge', 'rife-free-child' ); ?></button><div id="loading"></div>
	</div>
	<!-- Mel: End-->
                    ...
                    <?php while ( $the_query->have_posts() ) : $the_query->the_post();
                        ...
                        <tr>
                            ...
                            <td>
								...
								<!--Mel: 11/05/22: Start-->
								<input type="hidden" id="wallet" name="user_wallet" value="<?php echo get_user_meta( $user_id, 'wallet_address', true ); ?>" />
								<input type="hidden" id="token-id" name="token_id" value="<?php echo get_post_meta( absint( get_the_ID() ), '_gamipress_token_id', true ); ?>" />
								<script>
									document.getElementById("send-sbt-button").style.display = 'block';
								</script>
								<!--Mel: End-->
